...
def add_sentence_and_transcript(dataset, audio_to_text):
    sentences = [audio_to_text[os.path.splitext(os.path.basename(item['audio']['path']))[0]]
                 for item in dataset]
...
def main():
    ...
    parser.add_argument('--split-ratio', type=float, default=0.3,
                        help='Test+validation split ratio (e.g., 0.3 means 70/15/15)')
    parser.add_argument('--audio-dir', type=str, default='wav',
                        help='Directory containing audio files')
    parser.add_argument('--transcript-file', type=str, default=os.path.join('transcript','transcript.txt'),

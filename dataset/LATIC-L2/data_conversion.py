...
def main():
    ...
    audio_filenames = [os.path.splitext(os.path.basename(item["audio"]["path"]))[0] for item in tqdm(data["train"], desc="Extracting file names")]

    # 3) Extract transcripts
    scripts = {
        os.path.join(base_path, "data", "SCRIPT", "Actual_text", "total_transcript.txt"): (
            os.path.join(metadata_dir, "actual_wav.txt"),
            os.path.join(metadata_dir, "actual_transcript.txt"),
        ),
        os.path.join(base_path, "data", "SCRIPT", "Suppose_text", "total_transcript.txt"): (
            os.path.join(metadata_dir, "suppose_wav.txt"),
            os.path.join(metadata_dir, "suppose_transcript.txt"),
        ),
    }
...
def parse_args():
    parser = argparse.ArgumentParser(description="Prepare and split the LATIC-L2 Mandarin audio-transcript dataset")
    parser.add_argument(
        "--metadata-dir",
        type=str,
    ...
    parser.add_argument("--split-ratio", type=float, default=0.3, help="Proportion of the dataset to reserve for (test+validation).")
    parser.add_argument("--output-path", type=str, required=True, help="Directory where the final DatasetDict will be saved")

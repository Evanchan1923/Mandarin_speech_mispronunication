...
def _parse_gpu_ids(raw_value):
    ...
    return [int(x.strip()) for x in str(raw_value).split(",") if x.strip()]
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        for i in range(ngroups):
            ...
            loss = F.ctc_loss(
                ...
                zero_infinity=getattr(model.config, "ctc_zero_infinity", True),
...
class TrainSAModel:
    ...
    def __init__(self, config_file):
        ...
        self.requested_num_gpus = int(hardware_config.get("num_gpus", 1))
        self.gpu_ids = _parse_gpu_ids(hardware_config.get("gpu_ids", ""))

        if self.requested_num_gpus < 0:
            raise ValueError("hardware.num_gpus must be 0 or a positive integer.")
        if self.gpu_ids and self.requested_num_gpus not in (0, len(self.gpu_ids)):
            raise ValueError("hardware.num_gpus must match the number of entries in hardware.gpu_ids, or be set to 0.")

        if self.gpu_ids:
            os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(str(gpu_id) for gpu_id in self.gpu_ids)
        elif self.requested_num_gpus > 0:
            os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(str(gpu_id) for gpu_id in range(self.requested_num_gpus))
        else:
            os.environ["CUDA_VISIBLE_DEVICES"] = ""

        if self.requested_num_gpus > 0 and torch.cuda.is_available():
            self.accelerate = True
            self.device = torch.device("cuda")
            self.n_devices = torch.cuda.device_count()
            requested_visible_gpus = len(self.gpu_ids) if self.gpu_ids else self.requested_num_gpus
            if self.n_devices < requested_visible_gpus:
                raise ValueError(f"Requested {requested_visible_gpus} GPU(s), but only {self.n_devices} visible GPU(s) are available.")
        else:
        ...
        self.ctc_zero_infinity = config["training"].get("ctc_zero_infinity", True)
        ...
        self.eval_model_path = config["evaluation"]["eval_model_path"]

        if self.n_devices > 1:
            logger.info(f"Multi-GPU training enabled with {self.n_devices} GPUs.")
        elif self.device.type == "cuda":
            logger.info("Single-GPU training enabled.")
        else:
            logger.info("CPU training enabled.")

    def _get_per_device_train_batch_size(self):
        if self.n_devices <= 1:
            return self.train_batch_size
        if self.train_batch_size < self.n_devices:
            raise ValueError(f"training.train_batch_size ({self.train_batch_size}) must be at least the number of GPUs ({self.n_devices}).")

        per_device_batch_size = self.train_batch_size // self.n_devices
        effective_global_batch = per_device_batch_size * self.n_devices
        if effective_global_batch != self.train_batch_size:
            logger.warning(
                "training.train_batch_size=%s is not divisible by %s GPUs; using per-device batch size %s (effective global batch size %s).",
    ...
    def load_attribute_list(self):
        try:
            ...
            raise FileNotFoundError(f"The list of attribute file {self.attribute_list_file} is not found")
    ...
    def load_p2a_map(self):
        try:
            ...
            raise FileNotFoundError(f"The phoneme to attribute map file {self.phoneme2att_map_file} is not found")
        ...
        def mapToken(phList, mappers=self.phoneme_binary_mappers):
            ...
            for mapper in mappers:
                ...
                for p in phList.split():
                    ## Check if the script is running the tone training/evaulation
                    if "tone" in self.phoneme_column:
                        p = int(p)
    ...
    # Should run with batched=True                           IIIIIIIIIIIII
    def _prepare_dataset(self, batch):
        ...
        assert len(sampling_rates) == 1 and list(sampling_rates)[0] == 16000, f"Make sure all inputs have the same sampling rate of {self.processor.feature_extractor.sampling_rate}."
    ...
    def _load_diphthongs_to_monophthongs_map(self):
        with open(self.diphthongs_to_monophthongs_map_file, "r", encoding="utf-8") as f:
    ...
    def _decouple_diphthongs(self, batch):
        # No need for word boundaries, just match exact phonemes
        pattern = r"|".join(re.escape(diph) for diph in self.diphthongs_to_monophthongs_map.keys())

        # Replace diphthongs with corresponding monophthongs
    ...
    def preprocess_data(self, data, bTraining=True):
        ...
        if self.decouple_diphthongs:
            if self.diphthongs_to_monophthongs_map_file:
                print("Decoupling Diphthongs running")
                self._load_diphthongs_to_monophthongs_map()
            else:
                logger.error(
                    "decouple_diphthongs is set to True but to mapping file is provided, please explicitly set decouple_diphthongs to False or provide a mapping file in diphthongs_to_monophthongs_map_file"
                )
                raise FileNotFoundError
            data = data.map(self._decouple_diphthongs, batched=False)

        print("no Decouple process")
    ...
    def prepare_trainer(self):
        ...
        self.model.config.ctc_zero_infinity = self.ctc_zero_infinity
        ...
        self.training_args = TrainingArguments(
            ...
            per_device_train_batch_size=self._get_per_device_train_batch_size(),
            ...
            # Jinghao New changes for training aguments
    ...
    def map_to_result(self, batch):
        input_values = self.processor(batch["audio"]["array"], sampling_rate=batch["audio"]["sampling_rate"], return_tensors="pt").input_values.to(self.device)
    ...
    def evaluate_SA_model(self, eval_data=None, eval_parts=None, suffix=None, phoneme_column=None):
        ...
        self.trained_model_path = self.eval_model_path
        ...
        if test_data_loaded:
            ...
            with open(join(self.working_dir, f"results_{suffix}.txt"), "w") as f:
                ...
                # Open a new CSV file for error types
                with open(join(self.working_dir, f"error_types_{suffix}.csv"), "w", newline="") as csv_file:
                    csv_writer = csv.writer(csv_file)

                    # Write the header for the CSV
                    csv_writer.writerow(["Group", "Substitutions", "Deletions", "Insertions", "Hits", "total"])
                    ...
                    if isdict:
                        for dataset in self.results:
                            for g in range(ngroups):
                                ...
                                # Compute AER and error types
                                aer = metric._compute(predictions=pred, references=target)
                                subs, dels, ins, hits = metric.compute_types(predictions=pred, references=target)
                                total = subs + dels + hits

                                # Write to the main file
                                print("{} group {} AER: {:.5f}".format(dataset, self.groups[g][0].replace("p_", ""), aer), file=f)
                                print("Error type: Sub {}, Del {}, Insert {}. Correct hits {} Total is {}".format(subs, dels, ins, hits, total), file=f)
                                print("\n", file=f)

                                # Write to the CSV file
                                csv_writer.writerow([self.groups[g][0].replace("p_", ""), subs, dels, ins, hits, total])

...
class TrainPhModel:
    ...
    def _prepare_dataset(self, batch):
        ...
        assert len(sampling_rates) == 1 and list(sampling_rates)[0] == 16000, f"Make sure all inputs have the same sampling rate of {processor.feature_extractor.sampling_rate}."
    ...
    def preprocess_data(self, data, bTraining=True):
        ...
        if self.decouple_diphthongs:
            ...
            data = data.map(
                self._process_diphthongs,
                batched=False,
                fn_kwargs={"phoneme_column": self.phoneme_column, "decouple": True},
    ...
    def evaluate_model(self, eval_data=None, eval_parts=None, suffix=None, phoneme_column=None, decouple_diph=None):
            ...
            with open(join(self.working_dir, f"results_{suffix}.txt"), "w") as f:
                if isdict:
                    for dataset in self.results:
                        print(
                            "Test PER: {:.3f}".format(self.metric.compute(predictions=self.results[dataset]["pred_str"], references=self.results[dataset][self.phoneme_column])),
                            file=f,
                        )
                else:
                    print(
                        "Test PER: {:.3f}".format(self.metric.compute(predictions=self.results["pred_str"], references=self.results[self.phoneme_column])),
                        file=f,
                    )

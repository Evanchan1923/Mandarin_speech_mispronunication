...
class transcribe_SA:
    ...
    def check_identical_phonemes(self, df_p2att):
        ...
        if identical_phonemes:
            logger.warning(
                "The following phonemes has identical phonological features given the phonological features used in the model. If using fixed weight layer, these phonemes will be confused with each other"
            )
    ...
    def read_phoneme2att(self, p2att_file):
        ...
        if not_supported:
            logger.warning(
                f"Attribute/s {','.join(not_supported)} is not supported by the model {self.model_path} and will be ignored. To get available attributes of the selected model run transcribe --model_path=/path/to/model print_availabel_attributes"
            )
        ...
        for i, r in df_p2att.iterrows():
            ...
            for att in r.index.values:
                if f"p_{att}" not in self.processor.tokenizer.vocab:
                    logger.warn(
                        f"Attribute {att} is not supported by the model {self.model_path} and will be ignored. To get available attributes of the selected model run transcribe --model_path=/path/to/model print_availabel_attributes"
                    )
                    continue
                value = r[att]
                if value == 0:
    ...
    def transcribe(self, audio_file, attributes="all", phonological_matrix_file=None, human_readable=True):
        ...
        if not target_attributes:
            logger.error(
                f"None of the given attributes is supported by model {self.model_path}. To get available attributes of the selected model run transcribe --model_path=/path/to/model get_available_attributes"
            )
    ...
    def evaluate_dataset(
        ...
        diph2mono_file=None,
        decouple=True,
    ):  # If diph2mono is provided and decouple = True, both ref and pred phonemes will be decoupled, if decouple = False
